  ...
  return (
    <div className="w-full h-fit bg-tertiary flex flex-row gap-4 sm:gap-8 lg:gap-12 items-center pl-5">
      <button
        ...
        className={`${limeLight.className} font-semibold text-5xl text-secondary hover:scale-125 hover:rotate-[370deg] transition-all duration-150`}
      >
        R
      </button>
      <div className="w-full h-fit bg-tertiary flex flex-row gap-4 sm:gap-6 lg:gap-12 items-center pl-5 py-3">
        {indexes.map((data, index) => (
          <button
            key={index}
            className={`${inter.className} font-semibold text-xl sm:text-2xl md:text-4xl text-secondary hover:translate-y-[10%] transition-all duration-100`}
            onClick={() => {
